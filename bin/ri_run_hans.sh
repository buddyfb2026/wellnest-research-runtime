...
agent="hans"
lane="competitor"
schema="$HOME/ri_db/schemas/competitor_discovery_signal.schema.json"
...
IFS=$'\t' read -r job_id job_type lane input_ref idempotency_key attempt_count <<< "$claim"

session_id="$("$HOME/ri_db/bin/ri_make_session.sh" "$agent")"
session_dir="$HOME/ri_db/work/$agent/$session_id"

trap 'fail_job "$job_id" "$session_id" "unexpected_runtime_error" "hans wrapper failed unexpectedly"' ERR
...
cat > "$session_dir/input.json" <<JSON
{
  "job_id": "$job_id",
  "job_type": "$job_type",
  "lane": "$lane",
...
Discover competitor signals for: $input_ref
TXT

cat > "$session_dir/raw_output.txt" <<TXT
Synthetic scaffold output for competitor discovery on query: $input_ref
TXT
...
{
  "artifact_type": "competitor_discovery_signal",
  ...
  "agent": "hans",
  "query": "$input_ref",
  "company_name": "Synthetic Household App Co",
  "product_name": "Synthetic Family Planner",
  "category": "household management",
  "website_url": "https://example.com/synthetic_household_app",
  "app_store_urls": [
    "https://apps.apple.com/us/app/example/id123456789"
  ],
  "summary": "Synthetic scaffold competitor discovery output used to validate WellNest runtime wiring.",
  "evidence": [
    {
      "url": "https://example.com/synthetic_household_app",
      "kind": "seed",
      "title": "Synthetic Competitor Site",
      "snippet": "Placeholder evidence for initial competitor pipeline validation."
    }
  ],
  "verification_status": "unverified",
  "confidence_score": 0.5,
  "discovered_at": "$(ri_now_utc)",
  "content_hash": "",
  "idempotency_key": "$idempotency_key",
