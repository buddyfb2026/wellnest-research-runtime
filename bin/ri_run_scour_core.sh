...
agent="scour"
lane="creator"
schema="$HOME/ri_db/schemas/creator_discovery_signal.schema.json"
...
trap 'fail_job "$job_id" "$session_id" "unexpected_runtime_error" "scour wrapper failed unexpectedly"' ERR
...
Discover creator signals for: $input_ref
TXT

cat > "$session_dir/raw_output.txt" <<TXT
Synthetic scaffold output for creator discovery on query: $input_ref
TXT
...
cat > "$session_dir/normalized.json" <<JSON
{
  ...
  "query": "$input_ref",
  "platform": "web",
  "creator_name": "Synthetic Working Mom Creator",
  "handle": "@synthetic_creator",
  "profile_url": "https://example.com/synthetic_creator",
  "title": "Working Mom Household Systems",
  "summary": "Synthetic scaffold discovery output used to validate WellNest runtime wiring.",
  "evidence": [
    {
      "url": "https://example.com/synthetic_creator",
      "kind": "seed",
      "title": "Synthetic Creator Profile",
      "snippet": "Placeholder evidence for initial pipeline validation."
    }
  ],
