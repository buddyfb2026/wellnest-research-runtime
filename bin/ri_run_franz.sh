...
agent="franz"
lane="competitor"
schema="$HOME/ri_db/schemas/competitor_analysis.schema.json"
...
trap 'fail_job "$job_id" "$session_id" "unexpected_runtime_error" "franz wrapper failed unexpectedly"' ERR
...
company_name="$(python3 - <<'PY' "$source_path"
import json,sys
...
product_name="$(python3 - <<'PY' "$source_path"
import json,sys
obj=json.load(open(sys.argv[1]))
...
website_url="$(python3 - <<'PY' "$source_path"
import json,sys
obj=json.load(open(sys.argv[1]))
print(obj["website_url"])
PY
)"

cat > "$session_dir/prompt.txt" <<TXT
Analyze competitor discovery signal and extract competitor analysis from source artifact: $input_artifact_id
TXT

cat > "$session_dir/raw_output.txt" <<TXT
Synthetic scaffold competitor analysis for source artifact: $input_artifact_id
TXT
...
cat > "$session_dir/normalized.json" <<JSON
{
  ...
  "target_user": "Busy parents coordinating household logistics",
  "feature_map": [
  ...
  "strengths": [
    "Simple family coordination value proposition",
    "Clear planning use case"
  ],
  "weaknesses": [
    "May not deeply address mental load",
    "Likely fragmented household workflow coverage"
  ],
  "gaps": [
    "Proactive household orchestration",
    "Cross-workflow recommendations"
  ],
  "opportunities": [
    "Autopilot planning support",
    "End-to-end household reset flows"
  ],
  "evidence": [
    {
      "url": "$website_url",
      "kind": "source_artifact",
      "title": "Source competitor site",
      "snippet": "Derived from Hans discovery artifact."
    }
  ],
